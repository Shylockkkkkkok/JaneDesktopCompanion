...
一个安静停留在 Windows 桌面上的人物陪伴体（V0.1）。

JANE 不是一个普通软件窗口，而是一个**直接存在于桌面上的小人物**：背景透明、无边框、始终置顶、可拖动，会对鼠标悬停 / 点击 / 连续点击做出反应，偶尔通过气泡说一句话。
...
当前目标平台：Windows 10 / 11。代码尽量不绑定 Windows，便于未来扩展 macOS。
...
- 使用静态 PNG + CSS 动画，暂无 Live2D / 骨骼动画。
- 开机启动（Autostart）UI 已就位，但逻辑标记为 `TODO`，未接入 Windows 启动。
- 人物动画为一次性 CSS 关键帧，非循环动画序列。
- 托盘图标为运行时生成的占位圆点，后续替换正式图标。
- 气泡边缘切换为左右/上方三档启发式判断，未处理极端多显示器布局的精细避让。
...
- 专注陪伴 / 演唱会倒计时 / 纪念空间
- Live2D / PNG Sequence / Sprite Sheet 动画
- 更多台词与更丰富的状态系统（sleep / focus / concert）
- 开机启动接入
- macOS 支持
- 正式图标与人物素材替换
